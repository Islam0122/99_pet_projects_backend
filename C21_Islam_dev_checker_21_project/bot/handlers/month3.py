    ...
    try:
        ...
        if github_url.lower() in ['нет', 'no', 'skip', '-', '']:
            try:
                await message.bot.edit_message_caption(
                    chat_id=message.chat.id,
                    message_id=user_last_message.get(user_id),
                    caption="❌ *GitHub ссылка ОБЯЗАТЕЛЬНА!*\n\n"
        ...
        if not is_valid_github_url(github_url):
            try:
                await message.bot.edit_message_caption(
                    chat_id=message.chat.id,
                    message_id=user_last_message.get(user_id),
                    caption="❌ *Неверный формат GitHub ссылки!*\n\n"
        ...
        try:
            await message.bot.edit_message_caption(
                chat_id=message.chat.id,
                message_id=user_last_message.get(user_id),
                caption="⏳ *Создаем домашнее задание...*\n\n"
...
async def show_homework_detail(callback: types.CallbackQuery):
    ...
    lesson_display = homework.get('lesson_display', get_lesson_display_name(homework.get('lesson', '')))
    title = homework.get('title', '')
    task_condition = homework.get('task_condition', '')
    github_url = homework.get('github_url', '')
    grade = homework.get('grade')
    is_checked = homework.get('is_checked', False)
    created_at = homework.get('created_at', '')
    originality_check = homework.get('originality_check', '')
